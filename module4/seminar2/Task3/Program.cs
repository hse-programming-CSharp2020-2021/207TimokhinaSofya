...
namespace Task3
{
    class Program
    {
        static async Task Main()
        {
            ...
            var simbols = Enumerable.Range(1, random.Next(10, 20)).Select(x => new ConsoleSimbolStruct((char)random.Next('a', 'z'), random.Next(10, 100), random.Next(10, 100))).ToArray();
            Console.WriteLine(string.Join(Environment.NewLine, simbols));
            Data(simbols);
            await Json(simbols);
            Xml(simbols);
            Bin(simbols);
        ...
        static void Xml(ConsoleSimbolStruct[] simbols)
        {
            ...
            XmlSerializer formatter = new XmlSerializer(typeof(List<ConsoleSimbolStruct>));
        ...
        static void Data(ConsoleSimbolStruct[] simbols)
        {
            ...
            DataContractSerializer serializer = new DataContractSerializer(typeof(ConsoleSimbolStruct[]));
